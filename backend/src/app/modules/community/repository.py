...
async def set_post_reported(post_id: str, user_id: str) -> CommunityPost:
    ...
    await prisma.communitypost.update(where={"id": post_id}, data={"reported": True})
    post = await find_post(post_id)
    assert post is not None
    return post
...
async def set_comment_reported(comment_id: str, user_id: str) -> None:
    ...
    if not existing:
        await prisma.communitycommentreport.create(data={"commentId": comment_id, "userId": user_id})
