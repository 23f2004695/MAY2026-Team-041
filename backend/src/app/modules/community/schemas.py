...
class CommentOut(BaseModel):
    ...
    def from_prisma(comment, current_user_id: str | None = None) -> "CommentOut":
        ...
        reported_by_me = any(getattr(r, "userId", None) == current_user_id for r in reports) if current_user_id else False
...
def _build_comment_tree(comments: list, current_user_id: str | None = None) -> list[CommentOut]:
    by_id = {comment.id: CommentOut.from_prisma(comment, current_user_id=current_user_id) for comment in comments}
