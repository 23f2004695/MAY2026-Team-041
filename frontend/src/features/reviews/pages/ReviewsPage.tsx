import { MessageSquare, MessageSquareOff, Pencil, RotateCcw, Star, Trash2, Users } from 'lucide-react';
...
export function ReviewsPage() {
  ...
  const deletingReview =
    activeReviewsList.find((review) => review.id === deletingReviewId) ?? null;
    ...
    if (routeBookId) {
      ...
      Promise.all([apiGet<Book>(`/books/${routeBookId}`), getBookReviews(routeBookId)])
        ...
        .catch(() => {
          ...
          setBookReviews(EMPTY_BOOK_REVIEWS);
        });
    } else if (isStaff) {
      // 2. Staff Catalog Moderation View: Fetch all reviews
      getAllReviews()
        .then((data) => {
          if (!cancelled) setAllReviews(data);
        })
        .catch(() => {
  ...
  const refreshCurrentView = () => {
    if (routeBookId) {
      getBookReviews(routeBookId).then(setBookReviews).catch(() => {});
    } else if (isStaff) {
        ...
        <>
          <PageTitle
            ...
            description={
              selectedBook ? (
                <>
                  <Link to={bookLink(selectedBook.id)} className="font-semibold text-primary hover:underline">
                    {selectedBook.title}
                  </Link>{' '}
          ...
          <div className="flex flex-col gap-4">
            ...
            {bookReviews.items.length === 0 ? (
              <EmptyState
                ...
                description={t('reviews.empty.description', 'Be the first to share what you thought of this book.')}
              />
            ) : (
              <div className="flex flex-col gap-4">
                <div className="grid gap-4 sm:grid-cols-2">
                  {pagedBookReviews.map((review) => (
                    <ReviewCard
                      ...
                      onDelete={canModerate || review.is_own ? () => setDeletingReviewId(review.id) : undefined}
                    />
                  ))}
                </div>
                  ...
                  <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div className="w-full sm:w-72">
                      <SearchBar
                        ...
                        placeholder={t('reviews.member.searchPlaceholder', 'Search rated books by title...')}
                      />
                    </div>
                    ...
                    <div className="flex items-end gap-3">
                      <Select
                        ...
                        className="w-48"
                        options={[
                          { value: 'newest', label: t('reviews.member.sort.newest', 'Newest first') },
                          { value: 'ratingHigh', label: t('reviews.member.sort.ratingHigh', 'Highest rating') },
                          { value: 'ratingLow', label: t('reviews.member.sort.ratingLow', 'Lowest rating') },
                          { value: 'titleAsc', label: t('reviews.member.sort.titleAsc', 'Title (A–Z)') },
                          { value: 'titleDesc', label: t('reviews.member.sort.titleDesc', 'Title (Z–A)') },
                        ]}
                      />
                      ...
                      <Table>
                        <TableHeader>
                          <TableRow>
                            ...
                            <TableHead>{t('reviews.member.table.rating', 'My Rating')}</TableHead>
                          </TableRow>
                        </TableHeader>
                        <TableBody>
            ...
            <section className="flex flex-col gap-4">
              ...
              {writtenReviews.length === 0 ? (
                <Card>
                  <CardContent className="p-8 text-center">
                    <p className="text-muted-foreground">
                      {t('reviews.member.noWrittenReviews', "You haven't written any reviews yet.")}
                    </p>
                  </CardContent>
                </Card>
              ) : (
                        ...
                        <CardHeader className="flex-row items-start justify-between gap-3 space-y-0 pb-2">
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            <button
                              ...
                              aria-label={t('common.cards.review.deleteReview', 'Delete review')}
